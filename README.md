...
```
ファンダメンタル選定 → 直近の強さ確認 → テクニカル解析 → ニュース/SNSセンチメント補正 → 発注
```
...
新規買いは **3段のファネル**で絞り込みます（質 → 強さ → タイミング）:

```
① ファンダ足切り   割安・高収益・健全な銘柄に絞る（fundamental）
        ↓
② 直近の強さ       実際に上昇基調にある銘柄だけ通す（strength）
        ↓          ・直近 lookback_days のリターン ≥ min_recent_return
        ↓          ・株価 > ma_period 日移動平均（require_above_ma）
③ タイミング       テクニカル＋センチメントの総合スコアで売買判定
```

総合スコア（-1.0〜+1.0）= テクニカルスコア ×(1 − w) + センチメント ×w
（`w` = `sentiment.weight`）

- 新規買い: 総合スコア ≥ `buy_score_threshold` **かつ** ①ファンダ通過 **かつ** ②強さ通過
- 売り: 総合スコア ≤ `sell_score_threshold`（保有の損切り/利確も別途判定）
- 株数: 口座評価額 × `position_pct` を 100株単位で配分

> ②の強さフィルタにより「下落基調の銘柄をRSI等の逆張りで拾う」ことを防ぎ、
> 強い銘柄に絞ったうえでタイミングを計る設計に戻しています。`strength.enabled: false`
> で従来どおり無効化も可能。ライブ・バックテストの両方に同じ条件で効きます。
